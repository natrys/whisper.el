...
- =whisper-server-mode=: Choose between different transcription modes:
   ...
   - =remote=: In case you already have whisper.cpp HTTP server running in a remote machine
      - =whisper-server-baseurl=: Base URL for the whisper.cpp server (for both =local= or =remote= modes). Should be a full URL including protocol, host, and port (e.g. ~http://localhost:8642~ or ~https://whisper.example.com~)
...
(setq whisper-server-mode 'remote
      whisper-server-baseurl "http://192.168.0.101:8642")
#+end_src
